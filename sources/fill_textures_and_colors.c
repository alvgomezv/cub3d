...
/*   Updated: 2023/11/02 10:18:50 by alvgomez         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*fill_texture(char *word, t_map *map, char *line)
{
	...
	{
		free(texture);
		ft_error_and_free_map(map, "Texture file must be png", line);
	}
	close(fd);
	free(line);
	return (texture);
...
int	check_before_number(char *number, t_map *map, char *line)
{
	...
	while (number[i] < '0' || number[i] > '9')
	{
		if (number[i] < '0' || number[i] > '9')
			ft_error_and_free_map(map, "Color must be a number", line);
		i++;
	}
	return (i);
}
